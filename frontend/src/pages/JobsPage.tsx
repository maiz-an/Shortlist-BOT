...
export function JobsPage() {
  ...
  return (
    <div>
      ...
      <form className="mb-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-6" onSubmit={(e) => { e.preventDefault(); update({ q }); }}>
      ...
      {isLoading ? <Loading /> : error ? <ErrorState error={error} onRetry={() => refetch()} /> : !data || data.items.length === 0 ? (
        ...
        <>
          <div className={isFetching ? 'opacity-70 transition' : 'transition'}>
            <TableShell>
              <thead><tr><Th>Job</Th><Th>Match</Th><Th>Recommended CV</Th><Th>Source</Th><Th>Posted</Th><Th>Status</Th></tr></thead>
              <tbody>
                {data.items.map((j, idx) => (
                  <motion.tr key={j.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(idx, 12) * 0.025, duration: 0.25 }} className="hover:bg-slate-50">
                    ...
                    <Td className="text-slate-600">{j.analysis?.recommendedCv?.name ?? '—'}</Td>
                    <Td>{[...new Set(j.sourceListings.map((s) => s.jobSource.name))].join(', ')}</Td>
