...
- Applications are never sent without an explicit confirmation.

## Your part
- Keep `backend/.env`, `frontend/.env` and any Google `client_secret*.json` private. They are git-ignored; never commit or share them.
- Do not expose ports 5870 to 5872 to the network or the internet. There is no multi-user access control.
